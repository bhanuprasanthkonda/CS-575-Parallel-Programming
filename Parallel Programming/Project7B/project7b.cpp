...
main( int argc, char *argv[ ] )
{
    ...
    if( me == BOSS )
    {
        for( int dst = 0; dst < NumCpus; dst++ )
        {
            if( dst == BOSS )
                continue;
            // Boss send the unprocessed data
            MPI_Send( &BigSignal[dst*PPSize], PPSize+MAXSHIFTS, MPI_FLOAT, dst, 'u', MPI_COMM_WORLD );
        }
    }
    else
    {
        // Receiver receiving the unprocessed data
        MPI_Recv( PPSignal, PPSize+MAXSHIFTS, MPI_FLOAT, BOSS, 'u', MPI_COMM_WORLD, &status );
    }
    ...
    {   // receiver sending the processed data
        MPI_Send( PPSums, MAXSHIFTS, MPI_FLOAT, BOSS, 'p', MPI_COMM_WORLD );
    }
    ...
    if( me == BOSS )
    {
        ...
        for( int src = 0; src < NumCpus; src++ )
        {
            if( src == BOSS )
                continue;
            // boss receiving the processed data
            MPI_Recv( tmpSums, MAXSHIFTS, MPI_FLOAT, src, 'p', MPI_COMM_WORLD, &status );
            for( int s = 0; s < MAXSHIFTS; s++ )
                BigSums[s] += tmpSums[s];
        }
    }
