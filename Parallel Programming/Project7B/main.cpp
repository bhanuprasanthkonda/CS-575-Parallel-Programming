...
main( int argc, char *argv[ ] )
{
    ...
    {   // processors are send the data back to the BOSS Processor
    ...
    if( me == BOSS )
    {
        ...
        for( int src = 0; src < NumCpus; src++ )
        {
            if( src == BOSS )
                continue;
            // Receiving from the processors and finding the final sum
    ...
    if( me == BOSS )
    {
        ...
        // for csv generation
        fprintf(stderr, "%d,%.2lf,%d\n", NumCpus, performance, NUMELEMENTS);
    }
