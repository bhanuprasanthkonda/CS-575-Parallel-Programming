...
main( int argc, char *argv[ ] )
{
    ...
    float *hAnswer = new float[ NUM_WORK_GROUPS ];
    ...
    size_t answerSize = NUM_WORK_GROUPS * sizeof(float);
    ...
    cl_mem dAnswer = clCreateBuffer( context, CL_MEM_READ_WRITE, answerSize, NULL, &status );
	if( status != CL_SUCCESS )
		fprintf( stderr, "clCreateBuffer failed (3)\n" );
	...
	status = clEnqueueWriteBuffer( cmdQueue, dAnswer, CL_FALSE, 0, answerSize, hAnswer, 0, NULL, NULL );
	if( status != CL_SUCCESS )
	...
	status = clSetKernelArg( kernel, 2, sizeof(cl_mem), &dAnswer );
	if( status != CL_SUCCESS )
	...
	status = clEnqueueReadBuffer( cmdQueue, dAnswer, CL_TRUE, 0, answerSize, hAnswer, 0, NULL, NULL );
	if( status != CL_SUCCESS )
